...
- [x] Задача 2.6 Переход с мок-эмбеддингов на реальные вычисления: замена фейкового сервиса на реально работающий (2026-01-02: реализованы оба подхода, добавлен бенчмарк)
  - [x] Подзадача 2.6.1 Реализация локального подхода: интеграция sentence-transformers (all-MiniLM-L6-v2 или аналогичная модель) (2026-01-02: реализовано в EmbeddingService)
  ...
  - [x] Подзадача 2.6.3 Сравнение подходов: бенчмарк скорости и стоимости (локальный vs Gemini) на тестовом наборе новостей (2026-01-02: создан scripts/benchmark_embeddings.py)
  - [x] Подзадача 2.6.4 Выбор оптимального решения: на основе результатов сравнения определить предпочтительный подход (2026-01-02: рекомендуется local для production, см. docs/embeddings_setup.md)
  - [x] Подзадача 2.6.5 Интеграция выбранного решения: замена мок-сервиса, обновление `EmbeddingService`, добавление env-переменных для переключения (2026-01-02: реализовано, используется EMBEDDING_BACKEND env var)
  - [x] Подзадача 2.6.6 Миграция существующих данных: пересчет embeddings для всех новостей без embedding (опционально, батчами) (2026-01-02: создан scripts/recompute_embeddings.py)
