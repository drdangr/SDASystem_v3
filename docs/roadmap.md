...
- [x] Задача 2.3 Подключение NER/actors через LLM (или модель)
  ...
  - [ ] Подзадача 2.3.7 Пересчет `story.top_actors` при обновлении акторов/новостей (проверить текущую реализацию)
- [ ] Задача 2.4 Timeline на базе EventExtraction (facts/opinions): извлечение событий из news и сборка таймлайна story (2025-12-15: уточнение цели)
  - [ ] Подзадача 2.4.1 Извлечение событий из каждой новости (EventExtraction) + привязка к `story_id`
  - [ ] Подзадача 2.4.2 Привязка событий к `story.event_ids` и сохранение/обновление артефакта
...
- [ ] Задача 2.6 Embeddings: строить эмбеддинги для новостей и использовать их для similarity/сравнения
  - [ ] Подзадача 2.6.1 Автоген `news.embedding` при отсутствии (из текста новости)
  - [ ] Подзадача 2.6.2 Использовать embeddings для `compute_news_similarities` (и при необходимости кэшировать результаты)
- [ ] Задача 2.7 Clustering: собирать новости в истории автоматически
  - [ ] Подзадача 2.7.1 Кластеризация `news` → `stories` при отсутствии `stories.json` (DBSCAN/graph components)
  - [ ] Подзадача 2.7.2 Сохранение/обновление `data/stories.json` как артефакта пайплайна
