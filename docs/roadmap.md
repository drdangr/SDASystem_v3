...
- [x] Задача 2.6 Переход с мок-эмбеддингов на реальные вычисления: замена фейкового сервиса на реально работающий (2026-01-02: полностью реализовано и протестировано)
  - [x] Подзадача 2.6.1 Реализация локального подхода: интеграция sentence-transformers (all-MiniLM-L6-v2 или аналогичная модель) (2026-01-02: реализовано в EmbeddingService, работает стабильно)
  - [x] Подзадача 2.6.2 Реализация облачного подхода: использование Gemini API для генерации embeddings (если доступно) (2026-01-02: реализовано, но Gemini embedding API в разработке, используется fallback)
  - [x] Подзадача 2.6.3 Сравнение подходов: бенчмарк скорости и стоимости (локальный vs Gemini) на тестовом наборе новостей (2026-01-02: создан scripts/benchmark_embeddings.py, пройден, результаты в docs/embeddings_benchmark_results.md)
  - [x] Подзадача 2.6.4 Выбор оптимального решения: на основе результатов сравнения определить предпочтительный подход (2026-01-02: рекомендуется local для production - 13 текстов/сек, высокое качество, см. docs/embeddings_benchmark_results.md)
  - [x] Подзадача 2.6.5 Интеграция выбранного решения: замена мок-сервиса, обновление `EmbeddingService`, добавление env-переменных и UI для переключения (2026-01-02: реализовано, добавлена настройка в LLM Settings модалку, API endpoints, убран fallback на mock)
  - [x] Подзадача 2.6.6 Миграция существующих данных: пересчет embeddings для всех новостей без embedding (опционально, батчами) (2026-01-02: создан scripts/recompute_embeddings.py, успешно пересчитаны все 15 новостей с mock на local)
